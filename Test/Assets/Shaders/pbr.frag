...
float shadow(sampler2D lightmap, vec4 coord) {
  ...
  scene_z -= fwidth(scene_z) * 2;
  // ivec2 size = textureSize(lightmap, 0);
  float t = 0, c = 0;
  vec2 step = fwidth(coord.xy) / 4.0;
  int size = 2;
...
vec4 fragment (in FragData frag) {
  ...
  vec3 emit = pow(texture(emission, frag.uv0).rgb, vec3(2)) * 50;
  ...
  vec3 light = scene.lights[0].color.rgb * scene.lights[0].color.a * 2;
  ...
  reflected += texture360(environment, normalize(reflect(-V, N))).rgb;
